    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # ── NVIDIA NIM (OpenAI-compatible) ──────────────
                ...
                model = model_override or "meta/llama-3.1-8b-instruct"
            ...
            # ── AMAZON NOVA (OpenAI-compatible) ─────────────
            elif provider == "nova":
                model = model_override or "nova-lite-v1"
                ...
                if resp.status_code != 200:
                    err_text = resp.text[:300]
                    return {"text": "", "model": model, "error": f"Nova {resp.status_code}: {err_text}"}
                result = resp.json()
                try:
                    text = result['choices'][0]['message']['content']
                    return {"text": text, "model": model}
                except (KeyError, IndexError):
                    return {"text": "", "model": model, "error": "Failed to parse Nova response"}
