...
import { BookOpen, Zap, Thermometer, Waves, Layers, ChevronDown, ChevronUp, CheckCircle, type LucideIcon } from 'lucide-react';
...
const PROBLEMS: Problem[] = [
  {
    ...
    solve: (v) => {
      if (v.X >= 1 || v.X < 0 || v.k <= 0 || v.v0 <= 0 || v.ca0 <= 0) {
        return {
          answer: 'Invalid inputs: Conversion must be in [0, 1), k, v₀, CA₀ must be > 0',
          steps: ['Error: Please check constraints on input parameters.']
        };
      }
  ...
  },
  {
    ...
    solve: (v) => {
      ...
      if (dt1 <= 0 || dt2 <= 0) {
        return {
          answer: 'Invalid temperatures (temperature cross detected)',
          steps: [
            `Step 1: Counter-current ΔT₁ = Thi - Tco = ${v.thi} - ${v.tco} = ${dt1}°C`,
            `Step 2: ΔT₂ = Tho - Tci = ${v.tho} - ${v.tci} = ${dt2}°C`,
            `Error: Temperature differences must be strictly positive to avoid thermodynamics violations.`
          ]
        };
      }
      if (v.Q <= 0 || v.U <= 0) {
        return {
          answer: 'Invalid inputs: Heat Duty (Q) and Overall U must be > 0',
          steps: ['Error: Please check constraints on input parameters.']
        };
      }
      ...
      return {
        ...
        steps: [
          `Step 1: Counter-current ΔT₁ = Thi - Tco = ${v.thi} - ${v.tco} = ${dt1}°C`,
          `Step 2: ΔT₂ = Tho - Tci = ${v.tho} - ${v.tci} = ${dt2}°C`,
          `Step 3: LMTD = (ΔT₁ - ΔT₂) / ln(ΔT₁/ΔT₂)` + (Math.abs(dt1 - dt2) < 0.01 ? ` ≈ ΔT₁ (balanced flow)` : ` = (${dt1} - ${dt2}) / ln(${dt1}/${dt2})`),
          `Step 4: LMTD = ${lmtd.toFixed(2)}°C`,
  ...
  },
  {
    ...
    solve: (v) => {
      if (v.D <= 0 || v.L <= 0 || v.v < 0 || v.rho <= 0 || v.f < 0) {
        return {
          answer: 'Invalid inputs: Diameter, length, density, friction factor must be > 0',
          steps: ['Error: Please check constraints on input parameters.']
        };
      }
  ...
  },
  {
    ...
    solve: (v) => {
      if (v.y1 <= 0 || v.y2 <= 0 || v.y1 <= v.y2 || v.A <= 0) {
        return {
          answer: 'Invalid inputs: Compositions must be > 0 and y₁ > y₂, A must be > 0',
          steps: ['Error: Please check constraints on input parameters.']
        };
      }
      let N = 0;
      ...
      if (Math.abs(v.A - 1) < 0.001) {
        N = v.y1 / v.y2 - 1;
        steps = [
          `Step 1: Kremser equation for A = 1: N = y₁/y₂ - 1`,
          `Step 2: y₁/y₂ = ${v.y1}/${v.y2} = ${(v.y1 / v.y2).toFixed(1)}`,
          `Step 3: N = ${(v.y1 / v.y2).toFixed(2)} - 1 = ${N.toFixed(2)}`,
          `Step 4: Recovery = (y₁-y₂)/y₁ × 100 = ${recovery.toFixed(1)}%`,
        ];
      } else {
        const term = (v.y1 / v.y2) * (1 - 1 / v.A) + 1 / v.A;
        if (term <= 0) {
          return {
            answer: 'Calculation error: Kremser denominator argument is negative. Solvent flow rate is too low.',
            steps: ['Error: Check that absorption factor A and compositions are physically feasible.']
          };
        }
        N = Math.log(term) / Math.log(v.A);
        steps = [
          `Step 1: Kremser equation: N = ln[(y₁/y₂)(1-1/A) + 1/A] / ln(A)`,
          `Step 2: y₁/y₂ = ${v.y1}/${v.y2} = ${(v.y1 / v.y2).toFixed(1)}`,
          `Step 3: 1 - 1/A = 1 - 1/${v.A} = ${(1 - 1 / v.A).toFixed(4)}`,
          `Step 4: Numerator = ln[(${(v.y1 / v.y2).toFixed(1)})(${(1 - 1 / v.A).toFixed(4)}) + ${(1 / v.A).toFixed(4)}] = ${Math.log(term).toFixed(4)}`,
          `Step 5: N = ${Math.log(term).toFixed(4)} / ${Math.log(v.A).toFixed(4)} = ${N.toFixed(2)}`,
  ...
  },
  {
    ...
    solve: (v) => {
      if (v.X >= 1 || v.X < 0 || v.k <= 0 || v.v0 <= 0) {
        return {
          answer: 'Invalid inputs: Conversion must be in [0, 1), k and v₀ must be > 0',
          steps: ['Error: Please check constraints on input parameters.']
        };
      }
  ...
  },
  {
    ...
    solve: (v) => {
      if (v.eta <= 0 || v.eta > 1 || v.Q <= 0 || v.H <= 0 || v.rho <= 0) {
        return {
          answer: 'Invalid inputs: Efficiency must be in (0, 1], flow, head, density must be > 0',
          steps: ['Error: Please check constraints on input parameters.']
        };
      }
