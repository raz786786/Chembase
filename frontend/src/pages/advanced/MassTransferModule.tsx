...
function RigorousMcCabeThiele() {
  ...
  const getYeq = useCallback((x: number) => (a * x) / (1 + (a - 1) * x), [a]);
  const getXeq = useCallback((y: number) => y / (a - y * (a - 1)), [a]);

  const data = useMemo(() => {
    const eqCurve: { x: number; y_eq: number; y_x: number }[] = [];
    let lines: { x: number; y_rect?: number; y_strip?: number; y_q?: number }[] = [];
    ...
    if (Math.abs(fq - 1.0) < 0.001) {
      xi = xf;
      yi = (r + 1) !== 0 ? (r / (r + 1)) * xi + xd / (r + 1) : xd;
    } else {
      const m1 = (r + 1) !== 0 ? r / (r + 1) : 1;
      const b1 = (r + 1) !== 0 ? xd / (r + 1) : 0;
      const m2 = fq / (fq - 1);
      const b2 = -xf / (fq - 1);
  ...
  const getStages = () => {
    ...
    while (current_x > xb && stageCount < 30) {
      ...
      if (next_x > xi) {
        ...
        const m_strip = Math.abs(xi - xb) > 1e-5 ? (data.yi - xb) / (xi - xb) : 1;
...
function ExtractionCalc() {
  ...
  let N = NaN;
  const denom = xr - ys / mv;
  if (denom !== 0 && xf > xr && mv > 0) {
    if (A > 0 && Math.abs(A - 1) >= 0.001) {
      const term = ((xf - ys / mv) / denom) * (1 - 1 / A) + 1 / A;
      if (term > 0) {
...
function DryingCalc() {
  ...
  if (!isNaN(ms) && !isNaN(xi) && !isNaN(xf) && !isNaN(xc) && !isNaN(rc) && !isNaN(a) && a > 0 && rc > 0) {
    if (xf > Xe) {
      ...
      let t_falling = 0;
      if (xi > xc) {
        t_const = (ms * (xi - xc)) / (rc * a);
        if (xf < xc) {
          t_falling = ((ms * (xc - Xe)) / (rc * a)) * Math.log((xc - Xe) / (xf - Xe));
        }
      } else if (xi > Xe && xf < xi) {
        t_falling = ((ms * (xi - Xe)) / (rc * a)) * Math.log((xi - Xe) / (xf - Xe));
      }
      t_total = t_const + t_falling;
    }
  }
