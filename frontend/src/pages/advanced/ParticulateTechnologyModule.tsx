...
function Plot2D({ line, bars, logX, xLabel, yLabel, height = 210 }: {
  ...
  if (!xs.length) return <div className="text-xs text-surface-400 py-8 text-center">No data yet.</div>;
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6 items-start">
      <CalcCard title="Sieve analysis input" icon={Grid3x3}>
        <p className="text-[10px] font-black uppercase tracking-widest text-surface-400 mb-4">Mass retained per sieve (g)</p>
        <div className="space-y-2 mb-4">
          {r.rows.map(row => (
            <div key={row.mesh} className="flex items-center gap-3">
              <span className="w-24 text-xs font-bold text-surface-500 dark:text-surface-400">No. {row.mesh}</span>
              <span className="w-24 text-[10px] font-black text-surface-400">{row.aperture} μm</span>
              <input type="number" value={masses[row.mesh] ?? 0} onChange={e => setM(row.mesh, e.target.value)}
                className="flex-grow px-3 py-2 rounded-xl border border-surface-200 dark:border-surface-800 bg-surface-50 dark:bg-surface-900/50 text-sm font-bold text-surface-800 dark:text-surface-100 outline-none focus:border-violet-500" />
            </div>
          ))}
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowPass(p => !p)}
            className="px-4 py-2 rounded-xl bg-surface-100 dark:bg-surface-800 text-surface-500 dark:text-surface-400 text-[10px] font-black hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors">
      ...
      <div className="space-y-6">
        ...
        <div className="glass rounded-3xl border border-surface-200 dark:border-surface-800 p-5">
          ...
          <p className="text-[10px] text-surface-400 mt-2">Bars = mass retained on each sieve; violet line = cumulative % passing (finer than).</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="grid lg:grid-cols-2 gap-6 items-start">
          ...
          {BOND_MATERIALS.map((b, i) => (
            <button key={b.name} onClick={() => setMat(i)}
              className={`px-3 py-1.5 rounded-xl text-[10px] font-black transition-all ${mat === i
                ? 'bg-violet-600 text-surface-50 shadow-lg shadow-violet-500/25'
                : 'bg-surface-100 dark:bg-surface-800 text-surface-500 dark:text-surface-400 hover:bg-violet-100 dark:hover:bg-violet-900/40'}`}>
              {b.name} · {b.wi}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-6">
        <CalcCard title="Rittinger vs Kick comparison" icon={Boxes}>
          ...
          <p className="text-[10px] text-surface-400 mt-2">Rittinger & Kick constants shown are illustrative (scaled from Wi) so the three laws are comparable — real values need lab calibration.</p>
          <p className="text-xs text-surface-500 dark:text-surface-400 mt-4 leading-relaxed">
            All three predict <b className="text-surface-700 dark:text-surface-200">{fmt(RR, 0, '×')}</b> reduction from {fmt(Fv, 0, ' μm')} to {fmt(Pv, 0, ' μm')}. Rittinger (1887) suits fine grinding (surface area dominates), Kick (1885) suits coarse crushing (volume dominates); Bond (1952) is the accepted middle ground for most industrial mills.
          </p>
        </CalcCard>
        <CalcCard title="Jaw crusher" icon={Boxes}>
          ...
          <p className="text-xs text-surface-500 dark:text-surface-400 mt-4 leading-relaxed">
            <span>With the feed rate at <b className="text-surface-700 dark:text-surface-200">150 t/h</b>, the crusher must swallow particles up to the gape ({fmt(Gv, 0, ' mm')}) and discharge below the set ({fmt(Sv, 0, ' mm')}). Typical primary crushers run ratios of 4:1 to 8:1.</span>
          </p>
        </CalcCard>
      </div>
    </div>
      ...
      <div className="space-y-6">
        <CalcCard title="Hindered settling · thickening" icon={Waves}>
          <p className="text-xs text-surface-500 dark:text-surface-400 leading-relaxed mb-4">
            In a thickener the slurry is dense, so particles slow each other down. Richardson–Zaki: <b className="text-surface-700 dark:text-surface-200">v = vt·(1−ε)^n</b> with n ≈ 4.7 in the Stokes regime.
          </p>
          <div className="grid grid-cols-3 gap-3">
        ...
        <CalcCard title="Settling regimes" icon={Waves}>
          <div className="space-y-2">
            {[
              ...
              <div key={n} className="rounded-xl border border-surface-200 dark:border-surface-800 p-3 flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-black text-surface-700 dark:text-surface-200">{n}</p>
                  <p className="text-[11px] text-surface-400">{t}</p>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="grid lg:grid-cols-2 gap-6 items-start">
      ...
      <div className="space-y-6">
        <div className="glass rounded-3xl border border-surface-200 dark:border-surface-800 p-5">
          ...
          <p className="text-[10px] text-surface-400 mt-2">Lapple model: η = 1/(1+(d50/d)²). The S-curve steepens as d50 drops — sharp cut-off means a good separator.</p>
        </div>
    ...
    <div className="grid lg:grid-cols-2 gap-6 items-start">
      ...
      <div className="space-y-6">
        <div className="glass rounded-3xl border border-surface-200 dark:border-surface-800 p-5">
          ...
          <p className="text-[10px] text-surface-400 mt-2">The parabolic shape is the signature of cake filtration — the rate falls as the cake thickens.</p>
        </div>
        <FluidizationCard />
      </div>
    </div>
...
function FluidizationCard() {
  ...
  return (
    <CalcCard title="Minimum fluidization velocity" icon={Filter}>
      ...
      <p className="text-xs text-surface-500 dark:text-surface-400 mt-4 leading-relaxed">
        Below <b className="text-surface-700 dark:text-surface-200">U_mf</b> the bed is fixed; above it, the particles lift and the bed behaves like a boiling liquid (pressure drop = bed weight per area). Between U_mf and vt is the fluidization window — FCC reactors, fluid beds and pneumatic conveyors all live here.
      </p>
    </CalcCard>
  );
}
    ...
    <div className="animate-in fade-in duration-500">
      <div className="mb-8">
        <div className="flex items-center gap-4 mb-1">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500 to-primary-600 text-surface-50 flex items-center justify-center shadow-lg shadow-violet-500/25">
            <Grid3x3 className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-2xl font-black text-surface-900 dark:text-surface-50">Particulate Technology</h1>
            <p className="text-sm text-surface-500 dark:text-surface-400">Solids handling: size analysis, crushing & grinding, settling, cyclones, filtration and fluid beds.</p>
          </div>
        </div>
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black transition-all ${tab === t.id
                ? 'bg-violet-600 text-surface-50 shadow-lg shadow-violet-500/30'
                : 'bg-surface-50 dark:bg-surface-900/60 text-surface-500 dark:text-surface-400 border border-surface-200 dark:border-surface-800 hover:border-violet-400/50 hover:text-violet-600 dark:hover:text-violet-300'}`}>
